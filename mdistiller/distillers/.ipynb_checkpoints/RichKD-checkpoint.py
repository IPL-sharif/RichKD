...
class RichKD(Distiller):
    """Enriching Knowledge Distillation with Cross-Modal Teacher Fusion"""

    def __init__(self, student, teacher, cfg, num_data, CLIP):
        super(RichKD, self).__init__(student, teacher)
        self.ce_loss_weight = cfg.RICH.LOSS.CE_WEIGHT
        self.feat_loss_weight = cfg.RICH.LOSS.FEAT_WEIGHT
        self.logit_loss_weight = cfg.RICH.LOSS.LOGIT_WEIGHT
        self.CLIP = CLIP
        self.temperature = cfg.RICH.TEMPERATURE
        self.alpha = cfg.RICH.FUSION_ALPHA
        self.CLIP_DIM = cfg.RICH.FEAT.CLIP_DIM

        self.init_crd_modules(
            cfg.RICH.FEAT.STUDENT_DIM,
            cfg.RICH.FEAT.TEACHER_DIM,
            cfg.RICH.FEAT.DIM,
            num_data,
            cfg.RICH.NCE.K,
            cfg.RICH.NCE.MOMENTUM,
            cfg.RICH.NCE.TEMPERATURE,
        )
    ...
    def init_crd_modules(
        ...
        self.embed_c = Embed(self.CLIP_DIM, feat_dim) 
    ...
    def forward_train(self, image, target, index, contrastive_index, **kwargs):
        ...
        with torch.no_grad():
            ...
            logits_clip, features_clip = self.CLIP(index)
            features_clip = features_clip.float()


        loss_feature = 0
        if self.feat_loss_weight != 0:
            f_s = self.embed_s(features_student)
            f_t = self.embed_t(features_teacher)
            f_c = self.embed_c(features_clip)
        
            f_fused = F.normalize(self.alpha * f_t + (1 - self.alpha) * f_c, dim=1)
            loss_feature = self.feat_loss_weight * self.crd_loss(
                f_s,
                f_fused,
                index,
                contrastive_index,
        ...
        loss_logit = 0
        if self.logit_loss_weight != 0: 
            logits_fused= self.alpha * logits_teacher + (1 - self.alpha) * logits_clip
            loss_logit = self.logit_loss_weight * kd_loss(logits_student, logits_fused, self.temperature)
